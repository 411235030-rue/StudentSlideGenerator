    ...
    ILogger<GeminiDeckGenerationService> logger) : IDeckGenerationService
{
        ...
        CancellationToken cancellationToken)
    {
        var prompt =
    $"""
    你是資深簡報編輯與資訊架構師。此階段只分析文件並規劃「可直接製作成簡報」的大綱；
    不撰寫投影片全文、不提供設計建議，也不要輸出任何 schema 以外的文字。
    ...
    任務：
    1. 先辨識文件的主題、章節、核心概念、定義、公式、流程、比較、案例、圖表與結論。
    2. 以文件原有邏輯為優先；只有在原文結構鬆散時，才重組成容易理解的敘事。
    3. 每張投影片只能傳達一個核心訊息。定義、公式、流程、比較、案例與結論若內容足夠，必須拆成不同頁。
    4. 不得為湊頁數重複前一頁、加入空泛目錄頁，或把三個以上核心概念塞在同一頁。
    5. 敘事順序必須適合文件本身：
       - 概念型教材：背景／核心概念／方法或證據／應用／結論。
       - 流程型教材：目標／步驟／判斷點／結果或案例。
       - 研究或報告：問題／方法／證據／發現／意義。
       - 若文件已有清楚章節，優先保留其合理順序。
    6. concise 建議 6～8 頁；standard 建議 9～12 頁；detailed 建議 13～18 頁；
       auto 依文件密度在 8～16 頁間決定。
    7. recommendedSlideCount 必須等於所有 sections 的 slideCount 總和。
    8. keyPoints 必須是來源可支持的重點，不得補造數據、例子、引用、結論或專有事實。
    9. 若文件資訊不足或語意不明，保留原文的保守解讀，不得自行推論。
    10. PDF 中的圖片、圖表、表格與圖說若承載核心資訊，必須納入大綱判斷。
